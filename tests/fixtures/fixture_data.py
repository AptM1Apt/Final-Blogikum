...
def posts_with_unpublished_category(mixer, user):
    return mixer.cycle(N_TEST_POSTS).blend(
        "blog.Post", category__is_published=False, author=user
    )
...
def posts_with_future_date(mixer, user):
    ...
    return mixer.cycle(N_TEST_POSTS).blend(
        "blog.Post", pub_date=date_later_now, author=user
    )


@pytest.fixture
def posts_with_published_locations(
    mixer, published_locations, published_category, user
):
    return mixer.cycle(N_TEST_POSTS).blend(
        "blog.Post",
...
def posts_with_published_locations_from_another_published_category(
    ...
    return mixer.cycle(N_TEST_POSTS).blend(
        "blog.Post",
        category=another_published_category,
        location=mixer.sequence(*published_locations),
    )
...
@pytest.fixture
def post_with_unpublished_category(mixer, unpublished_category, user):
    return mixer.blend("blog.Post", category=unpublished_category, author=user)


@pytest.fixture
def post_with_published_location(mixer, published_location, published_category, user):
    return mixer.blend(
        "blog.Post",
...
def post_with_unpublished_location(
    mixer, unpublished_location, published_category, user
):
    return mixer.blend(
        "blog.Post",
        category=published_category,
...
def post_with_future_date(mixer, user):
    ...
    return mixer.blend("blog.Post", pub_date=date_later_now, author=user)
...
@pytest.fixture
def posts_with_author(mixer, user):
    return mixer.cycle(2).blend("blog.Post", author=user)
